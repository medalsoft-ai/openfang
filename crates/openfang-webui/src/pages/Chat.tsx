// Chat - Redesigned with improved message bubbles and input
...
export function Chat() {
  ...
  // Track the last user message sent (for restoring when switching sessions mid-stream)
  const lastSentUserMessageRef = useRef<string>('');
  // Per-agent streaming state - each agent has its own loading state
  const [streamingAgents, setStreamingAgents] = useState<Record<string, boolean>>({});
  const isStreaming = selectedAgentId ? streamingAgents[selectedAgentId] || false : false;
  const setIsStreaming = useCallback((value: boolean) => {
    if (selectedAgentId) {
      setStreamingAgents(prev => ({ ...prev, [selectedAgentId]: value }));
    }
  }, [selectedAgentId]);
  const [currentSession, setCurrentSession] = useState<Session | null>(null);

  // WebSocket connection state from session-aware hook
  ...
  }, [selectedAgentId, queryClient, t]);

  // Session-aware WebSocket hook
  const {
    connectionState: wsConnectionState,
    isConnected: wsIsConnected,
    sendMessage: wsSendMessage,
    reconnect: wsReconnect,
    getBufferedMessages: wsGetBufferedMessages,
  } = useSessionWebSocket({
    agentId: selectedAgentId,
    sessionId: currentSession?.session_id || null,
    onMessage: (msg) => {
      // Cast to the expected format for the message handler
      handleWebSocketMessage(msg as unknown as { type?: string; [key: string]: unknown });
    },
    onConnectionChange: (state) => {
      // When connection is established, check if there are no streaming messages
      // This handles the case where user switched away and back while message completed in background
      if (state === 'connected' && selectedAgentId) {
        setTimeout(() => {
          setStreamingAgents(prev => {
            const currentStreaming = prev[selectedAgentId];
            if (currentStreaming) {
              // Check messages ref for any streaming messages
              const hasStreamingMsg = messagesRef.current?.some(m => m.isStreaming || m.thinking);
              if (!hasStreamingMsg) {
                return { ...prev, [selectedAgentId]: false };
              }
            }
            return prev;
          });
  ...
  useEffect(() => {
    ...
    if (hasChanged) {
      ...
      // Check if agent is currently streaming for this session
      // If so, add a typing indicator to show user that agent is still outputting
      const isAgentStreaming = selectedAgentId ? streamingAgents[selectedAgentId] : false;
      const lastMessage = sessionMessages[sessionMessages.length - 1];
      const hasStreamingIndicator = lastMessage && (lastMessage.isStreaming || lastMessage.thinking);

      if (isAgentStreaming && !hasStreamingIndicator) {
        // Agent is streaming but session history not yet updated.
  ...
  // Note: WebSocket connection is now managed by useSessionWebSocket hook above
  // Page visibility handling is built into the hook
  ...
  const handleSend = useCallback(async (content: string) => {
    ...
    lastSentUserMessageRef.current = content;
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--soft-main)]">
      ...
      <ChatInput
        ...
        onStop={() => {
          if (selectedAgentId) {
            api.stopAgentRun(selectedAgentId).catch(() => {
              // Ignore errors
            });
          }
          setIsStreaming(false);
