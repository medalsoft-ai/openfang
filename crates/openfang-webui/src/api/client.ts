...
class APIClient {
  ...
  setAuthToken(token: string) {
    ...
    // Also set token on session connection manager
    import('./sessionConnection').then(({ sessionConnectionManager }) => {
      sessionConnectionManager.setAuthToken(token);
    });
  }
...
// Re-export wsManager from websocket.ts to maintain backward compatibility
import { wsManager as websocketManager } from './websocket';
